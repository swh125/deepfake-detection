...
const detectIPFromIPIP = async (): Promise<IPInfo | null> => {
  try {
    ...
    const response = await axios.get('https://freeapi.ipip.net/', {
      timeout: 5000,
    });
  ...
  try {
    ...
    await axios.get(`${apiBaseUrl}/api/v1/utils/detect-ip`, {
      params: { 
        ...
        country: ipInfo.country || undefined, // Pass country code
        // No longer pass city, province, coordinates
      },
      timeout: 10000,
    });
    ...
    // Save failure doesn't affect frontend display, only log
